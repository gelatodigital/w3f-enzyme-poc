...
export async function uniswapQuote(hre: any): Promise<number> {
  const quoterContract = new ethers.Contract(QUOTER_CONTRACT_ADDRESS, Quoter.abi, hre.ethers.provider);
...
async function getPoolConstants(hre: any): Promise<{
  ...
  const poolContract = new ethers.Contract(currentPoolAddress, IUniswapV3PoolABI.abi, hre.ethers.provider);
